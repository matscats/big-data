...
then
    ...
    echo siga para o script de setup do Debezium
    exit 1
fi
...
docker compose exec kafka /opt/kafka/bin/kafka-topics.sh --bootstrap-server localhost:9092 --create --topic musicdb-server
